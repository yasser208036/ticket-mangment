...
A ticket-management system: a Laravel 13 JSON API (`backend/`) and a Vue 3 SPA (`frontend/`), with MySQL 8 and Mailpit supplied by Docker Compose. Roles are **Admin**, **Agent**, and **User** — only a `user` account files tickets, and each is still matched to a `Requester` contact record by email (see "Authentication and authorization" below).
...
There are three roles — **Admin**, **Agent**, **User** — and only a `user` account may create a ticket (`TicketPolicy::create()`; `403` for admin and agent). `GET /tickets` and `GET /tickets/{ticket}` are scoped by role via `Ticket::scopeVisibleTo()`, not merely gated: an admin sees every ticket, an agent sees tickets assigned to them plus every unassigned ticket, and a user sees only tickets they created. No query parameter widens this scope, and `GET /tickets/stats` (`TicketStats::for($user)`) is scoped identically, reporting `scope` as `all` / `assigned` / `authored`. An agent may update, change the status of, or escalate only a ticket assigned to them.

Ticket assignment is asymmetric, not a shared endpoint: `TicketPolicy::assign()` allows only admins (`POST /tickets/{ticket}/assign`, target must be an active agent — an admin can never assign a ticket to another admin this way). There is no self-claim endpoint: an agent instead asks for an unassigned ticket via `POST /tickets/{ticket}/assignment-requests`, and an admin approves or declines it (`GET/POST /admin/assignment-requests/...`) — approving assigns the ticket through the same writer `/assign` uses (`App\Services\TicketAssignment`), so the activity trail's `assigned` row is indistinguishable in shape from a direct assign. The only ways a ticket gains an assignee are, in total: `/assign`, choosing an agent at ticket-creation time, escalation's own reassignment, and an approved assignment request. `GET /api/v1/agents` (id and name only, no email or role) lets a `user` account name an agent when filing a ticket, without being given `GET /admin/users`.

`Admin\UserController::destroy()` guards against orphaning tickets: deleting a user with any ticket where they are `created_by` or `assigned_to` (including soft-deleted, since `tickets.created_by` is `ON DELETE RESTRICT`) returns `422` with reassignment options unless the request supplies `reassign_to`, in which case both roles' tickets move to the target (logged to the activity trail) before the user and their tokens are deleted. The same controller and `Admin\UserPasswordController` (a separate admin-forces-password-reset endpoint, distinct from self-service `PATCH /auth/password`, requiring the *acting admin's* `current_password` and revoking all of the target's tokens) both route through `guardAgainstLockout()`/`otherActiveAdmins()`, which uses `lockForUpdate()` to block deleting, deactivating, demoting, or self-modifying the last active admin.
...
- Commit subjects are imperative and end with the Jira key, e.g. `Add ticket assignment and workload (TM-31)`; keep a commit scoped to one story. PRs must link the TM issue, summarize behavior, list verification commands run, include screenshots for UI changes, and call out any environment/migration/API-contract changes.
